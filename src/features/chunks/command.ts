...
import { createCachedContext, buildGraph } from '../../core/index.ts';
import type { ChunkResult } from '../../core/graph.ts';
import { pluralize, formatLines, resolveIsProject } from '../../core/utils.ts';
...
export async function chunksCommand(options: ChunksOptions): Promise<void> {
  const { ctx, persist } = createCachedContext(process.cwd(), { offline: options.offline });
  try {
